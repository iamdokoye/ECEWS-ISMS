document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function fetchData() {
        try {
            ...
            if (supervisor.user) {
                unitHeader.textContent = `${supervisor.user.unit || 'IT'} Students`;
    ...
    function renderStudents(students) {
        ...
        students.forEach((student, index) => {
            ...
            createTextElement(row, 'c6', student.area_of_interest || 'N/A');
    ...
    function filterStudents(students, nameFilter, institFilter, interestFilter) {
        return students.filter(student => {
            ...
            const interestMatch = !interestFilter || 
                (student.area_of_interest && student.area_of_interest.toLowerCase().includes(interestFilter.toLowerCase()));
